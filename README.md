...
│   ├── mip.py                           # MIP/低Fidelity fallbackによるCutTarget選択
...
- `gate_cutting.mip.MIPCutFinder` — NetworkX/SciPy がある環境ではMILP、ない環境では低Fidelity greedy fallbackで `CutTarget` を選択。
- `gate_cutting.mip.build_cut_graph()` — optional solver dependencies なしでテストできる軽量cut graphを構築。
- `gate_cutting.mip.select_low_fidelity_cut_targets()` — 低Fidelity edgeを悪い順に `CutTarget` 化するfallback。
...
`experiments/exp1/b1.py`, `experiments/exp2/b1.py`, `experiments/exp2/b2.py`, `experiments/exp2/check.py` は、これらの共通モジュールを使う形に移行し始めています。特に `MIPCutFinder` は `gate_cutting.mip` に切り出しています。`experiments/exp2/benchmark_phase1.py` / `benchmark_phase2.py` は、プロジェクトルートを `sys.path` に追加して `experiments.exp2.b2` を import する形に整理しています。
...
実装は TDD で進めます。現在の軽量テストは外部の `stim` / `qiskit` / `networkx` / `scipy` が未インストールでも fake や軽量 fallback を使って実行できます。`device.json` のパース、Stim変換、Gate Cutting 展開、MIP選択結果の `CutTarget` 化、実験スクリプトの共通モジュール利用を unit test で固定しています。
