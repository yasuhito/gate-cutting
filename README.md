...
`experiments/exp1/b1.py` と `experiments/exp2/b2.py` は、これらの共通モジュールを使う形に移行し始めています。
...
実装は TDD で進めます。現在の軽量テストは外部の `stim` / `qiskit` が未インストールでも fake を使って実行できます。`device.json` のパース、Stim変換、Gate Cutting 展開、実験スクリプトの共通モジュール利用を unit test で固定しています。
