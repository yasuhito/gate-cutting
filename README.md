...
- `gate_cutting.mip.MIPCutFinder` — 研究実行では NetworkX/SciPy の MILP で `CutTarget` を選択。依存不足時のみ低Fidelity greedy fallbackへ退避。
- `gate_cutting.mip.build_cut_graph()` — optional solver dependencies なしでテストできる軽量cut graphを構築。
- `gate_cutting.mip.select_low_fidelity_cut_targets()` — unit test / 依存不足時の安全網として、低Fidelity edgeを悪い順に `CutTarget` 化するfallback。
...
実装は TDD で進めます。現在の軽量テストは外部の `stim` / `qiskit` / `networkx` / `scipy` が未インストールでも fake や軽量 fallback を使って実行できます。`device.json` のパース、Stim変換、Gate Cutting 展開、MIP選択結果の `CutTarget` 化、実験スクリプトの共通モジュール利用、テストケースごとの「アサーション 1 つ原則」を unit test で固定しています。
