...
def run_b1_benchmark(n_qubits=10, 
                     depth=4, 
                     trials=10, 
                     shots=5000, 
                     max_cut_list=[0, 1, 2, 3, 4]):
    """
    b2.pyの実装を用いて、カット数 vs エラー率のベンチマークを実行する
    """
    # 結果格納用
    ...
    for k in max_cut_list:
        ...
        for _ in tqdm(range(trials), desc=f"Max Cuts = {k}"):
            ...
            if cut_pairs:
                ...
                # 精度が悪い場合は採用しない
                if (err_cut > 0.1):
                    #print("Cut method error too high; using Noisy result instead.")
                    #print(f"err_cut: {err_cut}, val_ideal: {val_ideal}, val_cut: {val_cut}, val_noisy: {val_noisy}")
                    val_cut = val_noisy
            else:
                #print("No cuts were made; using Noisy result for Cut method.")
                val_cut = val_noisy
...
def plot_benchmark_results(cut_counts, err_noisy, err_cut):
    ...
    plt.tight_layout()
    plt.savefig('phase2_benchmark_result.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    #MAX_CUT_LIST = [1, 2] # 横軸のステップ
    
    # 実行
    cuts, e_noisy, e_cut = run_b1_benchmark(N_QUBITS, DEPTH, TRIALS, SHOTS, MAX_CUT_LIST)
